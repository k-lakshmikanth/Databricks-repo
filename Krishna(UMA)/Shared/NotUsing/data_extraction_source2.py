...
metaData=spark.table("kpi_etl_analytics_conf.ctl_table_sync").where(" SourceName='source2' ").rdd.collect()

# COMMAND ----------

display(spark.table("kpi_etl_analytics_conf.ctl_table_sync").where(" SourceName='source2' "))
...
for row in metaData:
    ...
    .saveAsTable(f"{DestinationSchema}.{src_name}_{table_name}")
        ...
        spark.sql(f"update kpi_etl_analytics_conf.ctl_table_sync set WatermarkValue=current_timestamp() where TableName='{table_name}' and SchemaName='{src_name}' ")

        #Update the EffectiveDateColumnName in the metadata table for the next incremental load
        spark.sql(f"update kpi_etl_analytics_conf.ctl_table_sync set EffectiveDateColumnName='last_update_date' where TableName='{table_name}' and SchemaName='{src_name}' ")
# ====================================================================================================        

# COMMAND ----------

# %fs ls /mnt/adls_landing/bronze/source2_Contact/

# COMMAND ----------

# %sql

# describe history kpi_cloud_bl.source2_Contact
...
# select * from kpi_cloud_bl.source1_contact
